...
async function main() {
  ...
  const initTask = await prisma.task.create({
    data: {
      ...
      planningSummary: "ChatGPT 已确认 MVP 范围是项目/任务/Prompt/Review 的基础总控台，不需要先做复杂权限和自动执行。",
      implementationPlan: "初始化 Next.js；建立 Prisma schema；创建核心页面和 API；写 README 和验收说明；运行 lint/build。",
      codexBudgetNotes: "只阅读项目初始化相关文件，不重新讨论产品范围或技术栈。",
    ...
    data: [
      {
        ...
        planningSummary: "ChatGPT 已确认模板要把规划结果转成可执行字段，避免 Codex 反复追问上下文。",
        implementationPlan: "扩展任务字段；更新表单；更新 Prompt 生成器；验证创建任务 API。",
        codexBudgetNotes: "不要重新设计任务系统，只增强模板和 Prompt 输出。",
